...
#[tauri::command]
async fn launch_lm_studio() -> Result<(), String> {
    #[cfg(target_os = "macos")]
    {
        std::process::Command::new("open")
            ...
            .arg("LM Studio")
            .spawn()
            .map_err(|e| format!("Failed to launch LM Studio: {}. Make sure it's installed.", e))?;
    }
    Ok(())
}

#[tauri::command]
async fn launch_whisper_server(model_path: String, port: u16) -> Result<(), String> {
    #[cfg(target_os = "macos")]
    {
        // Check if whisper-server exists
        let whisper_path = std::path::PathBuf::from(&model_path)
            .parent()
            .and_then(|p| p.parent())
            .map(|p| p.join("build/bin/whisper-server"))
            .ok_or("Invalid model path")?;
        
        if !whisper_path.exists() {
            return Err("whisper-server not found. Make sure whisper.cpp is built.".to_string());
        }
        
        std::process::Command::new(&whisper_path)
            .arg("-m")
            .arg(&model_path)
            .arg("--host")
            .arg("127.0.0.1")
    ...
    let mut builder = tauri::Builder::default()
        .plugin(
            tauri_plugin_sql::Builder::default()
                .add_migrations("sqlite:koios.db", db::migrations())
                .build(),
        )
        ...
        // Updater disabled for koios - using GitHub releases directly
